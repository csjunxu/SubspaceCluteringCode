...
We evaluate the advantages of the proposed NNLSR method on subspace clustering problem. We compare with the state-of-the-art methods on four datasets including the Hopkins 155 dataset, the Extended Yale B dataset, the USPS and MNIST datasets. The Hopkins 155 motion dataset is consisted f 155 video sequences, each of which has two or three motion objects. The motion trajectives of each object is viewed as a subspace. The Extended Yale B dataset contains 16,128 images of 38 human subjects with 9 poses and 64 illumination conditions. The images are projected on a 60 dimensions space by PCA. Each class or subspace includes 64 imgaes which are resized into $32\times32$ pixels.
