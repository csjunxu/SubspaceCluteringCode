...
\textbf{Motivation 1}: Positive collaborative representation could achieve sparse representation since similar points are sparse while dissimilar points are dense.

\textbf{Motivation 2}: Positive supports are positive to self-reppresentation while negative supports are negative to self-representation.

\textbf{Motivation 3}: Better performance. Faster? 

\section{LSR Model}
The least squares regression (LSR) model \cite{} is proposed by Lu et al. can be formulated as follows:
\begin{equation}
\label{e1}
\min_{\bm{A}}
\|
